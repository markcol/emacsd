...
(use-package consult
  :defer t
  :bind
  ("C-t" . my/consult-imenu)
  ("C-x b" . consult-buffer)
  ("C-c b" . my/consult-projectile-buffer)
  ("M-g M-g" . consult-goto-line)

  :custom
  (consult-buffer-sources '(my/consult--source-buffer-outside-project
                            my/consult--source-project-buffer
                            my/consult--source-not-project-buffer))
  ...
  (defun my/consult-project-buffer ()
    ...
    (when-let (buffer (consult--multi '(consult--source-hidden-buffer
                                        my/consult--source-buffer-outside-project
                                        my/consult--source-project-buffer
                                        my/consult--source-not-project-buffer
  ...
  (defun my/consult-projectile-buffer ()
    "Select projectile buffers with consult.
Only shows file-based buffers."
    (interactive)
    (when-let (buffer (consult--multi `(my/consult--source-projectile-buffer)
                                      :require-match
                                      (confirm-nonexistent-file-or-buffer)
                                      :prompt "Switch to: "
                                      :history 'consult--projectile-buffer-history
  ...
  (defvar my/consult--source-buffer-outside-project
    `(:name "Buffer"
            :narrow   ?b
            :category buffer
            :face     consult-buffer
  ...
  (defvar my/consult--source-project-buffer
    `(:name "Project Buffers"
            ...
            :items
            ,(lambda ()
               (my/consult--buffer-query :sort 'visibility
                                            :directory 'project
                                            :as #'buffer-name)))
    "Project buffer candidate source for `consult-buffer'.")

  (defvar my/consult--source-not-project-buffer
    `(:name "Other Buffers"
  ...
  (defvar my/consult--source-projectile-buffer
    ...
    "Projectile buffer candidate source for `my/consult-projectile-buffer'.")

  ;; Custom versions of consult's buffer query functions which supports the
  ;; 'not-project directory value.
  (defun my/consult--normalize-directory (dir)
...
DIR can be project, not-project, nil or a path."
  ...
  (cl-defun my/consult--buffer-query (&key sort directory mode as predicate (filter t)
...
AS is a conversion function."
    ...
    ;; Consult.
    (when-let (root (or (my/consult--normalize-directory directory) t))
