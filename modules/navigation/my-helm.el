...
(use-package helm
  ...
  (helm-after-initialize . my/helm--toggle-source-header-line)
  (helm-minibuffer-set-up . my/helm--popwin-help-mode-off)
  (helm-cleanup . my/helm--popwin-help-mode-on)
  (helm-cleanup . my/helm--show-neotree-maybe)
  (helm-cleanup . my/helm--show-treemacs-maybe)
  ...
  (my/helm-auto-toggle-neotree t)
  (my/helm-auto-toggle-treemacs t)

  :init
  (defgroup my/helm nil
    "Siren specific settings for helm."
    :group 'helm)

  (defcustom my/helm-auto-toggle-neotree t
    "When t and automatically hide and show neotree when helm is activated."
    :type 'boolean
    :group 'my/helm)

  (defcustom my/helm-auto-toggle-treemacs t
    "When t and automatically hide and show treemacs when helm is activated."
    :type 'boolean
    :group 'my/helm)

  (defvar my/helm--did-hide-neotree nil)
  (defvar my/helm--did-hide-treemacs nil)
  ...
  (defun my/helm--toggle-source-header-line ()
  ...
  (defun my/helm--hide-neotree (&rest plist)
    (when (and my/helm-auto-toggle-neotree
               (not (eq helm-display-function 'helm-posframe-display))
               (fboundp 'neotree-hide)
               (fboundp 'neo-global--window-exists-p)
               (neo-global--window-exists-p))
      (setq my/helm--did-hide-neotree t)
      (neotree-hide)))

  (defun my/helm--show-neotree-maybe ()
    (when (and (fboundp 'neotree-show)
               my/helm--did-hide-neotree)
      (setq my/helm--did-hide-neotree nil)
      (run-with-timer 0.01 nil #'neotree-show)))

  (defun my/helm--hide-treemacs (&rest plist)
    (when (and my/helm-auto-toggle-treemacs
               (not (eq helm-display-function 'helm-posframe-display))
               (fboundp 'treemacs-get-local-window))
      (let ((win (treemacs-get-local-window)))
        (when win
          (setq my/helm--did-hide-treemacs t)
          (delete-window win)))))

  (defun my/helm--show-treemacs-maybe ()
    (when my/helm--did-hide-treemacs
      (setq my/helm--did-hide-treemacs nil)
      (run-with-timer 0.01 nil #'my/helm--show-treemacs)))

  (defun my/helm--show-treemacs ()
  ...
  :config
  (require 'helm-mode)
  (advice-add 'helm :before 'my/helm--hide-neotree)
  (advice-add 'helm :before 'my/helm--hide-treemacs))
...
(use-package helm-posframe
  ...
  (defun my/helm-posframe-clear-on-fullscreen (frame)
  ...
  (add-hook 'window-size-change-functions
            'my/helm-posframe-clear-on-fullscreen)
