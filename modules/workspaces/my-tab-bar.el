;;; my-tab-bar.el --- tab-bar configuration.
...
(use-package tab-bar
  ...
  (:map my/workspace-map
        ...
        ("r" . tab-rename)
        ("C-r" . tab-rename)
        ("n" . my/tab-bar-move-tab-right)
        ("p" . my/tab-bar-move-tab-left)
        ("s" . my/tab-bar-switch-to-or-create-tab)
        ("C-s" . my/tab-bar-switch-to-or-create-tab)
        ("e" . tab-switcher)
        ("C-e" . tab-switcher)
        ("u" . tab-undo)
        ("C-u" . tab-undo)
        (";" . my/tab-bar-echo-tab-list)
        ("C-;" . my/tab-bar-echo-tab-list)
        ("0" . my/tab-bar-switch-to-index)
        ("1" . my/tab-bar-switch-to-index)
        ("2" . my/tab-bar-switch-to-index)
        ("3" . my/tab-bar-switch-to-index)
        ("4" . my/tab-bar-switch-to-index)
        ("5" . my/tab-bar-switch-to-index)
        ("6" . my/tab-bar-switch-to-index)
        ("7" . my/tab-bar-switch-to-index)
        ("8" . my/tab-bar-switch-to-index)
        ("9" . my/tab-bar-switch-to-index)
  ...
  (tab-bar-show 1)
  (tab-bar-tab-group-format-function #'my/tab-bar-tab-group-format-default)
  (tab-bar-tab-hints t)
  ...
  (defun my/tab-bar-tab-group-format-default (tab i)
    (propertize
     (concat (if tab-bar-tab-hints (format "%d:" (- i 1)) "")
             (funcall tab-bar-tab-group-function tab))
     'face 'tab-bar-tab-group-inactive))

  (defun my/tab-bar-switch-to-or-create-tab (name)
    ...
    (let ((tab-names (mapcar (lambda (tab) (alist-get 'name tab))
      ...
      (if (member name tab-names)
        ...
        (my/tab-bar-new-named-tab name)))
    (tab-bar-select-tab (1+ (or (tab-bar--tab-index-by-name name) 0))))

  (defun my/tab-bar-new-named-tab (name)
  ...
  (defun my/tab-bar-switch-to-index (&optional arg)
...
ARG counts from 1."
  ...
  (defun my/tab-bar-echo-tab-list ()
    "Echo list of tabs"
    (interactive)
      ...
      (dolist (tab tabs)
        (add-to-list 'output
                     (concat (propertize (format "%d:" index)
                                         'face 'my/tab-bar-echo-index)
                             (propertize (alist-get 'name tab)
                                         'face (if (eq index current-index)
                                                   'my/tab-bar-echo-current
                                                 'my/tab-bar-echo-default)))
                     t)
        (setq index (1+ index)))

      (message "tabs: %s" (string-join output " "))))

  (defun my/tab-bar-echo-tab-list-advice (&rest _)
    (when my/tab-bar-echo-tab-list
      (my/tab-bar-echo-tab-list)))

  (advice-add 'tab-bar-close-tab :after #'my/tab-bar-echo-tab-list-advice)
  (advice-add 'tab-bar-move-tab-to :after #'my/tab-bar-echo-tab-list-advice)
  (advice-add 'tab-bar-new-tab-to :after #'my/tab-bar-echo-tab-list-advice)
  (advice-add 'tab-bar-rename-tab :after #'my/tab-bar-echo-tab-list-advice)
  (advice-add 'tab-bar-select-tab :after #'my/tab-bar-echo-tab-list-advice)
  (advice-add 'tab-switcher-select :after #'my/tab-bar-echo-tab-list-advice)
  (advice-add 'display-buffer-in-new-tab :after #'my/tab-bar-echo-tab-list-advice)
  (advice-add 'tab-bar-change-tab-group :after #'my/tab-bar-echo-tab-list-advice))
